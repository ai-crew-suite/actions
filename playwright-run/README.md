# `@ai-crew-suite/actions/playwright-run`
...
yarn turbo run build --filter=@ai-crew-suite/action-playwright-run
```

### Running Verification Tracks

```bash
yarn turbo run lint --filter=@ai-crew-suite/action-playwright-run
```
  ...
  deploy-gate:
    ...
    steps:
      - name: 🎭 Run Centralized Playwright Run Engine
        uses: ai-crew-suite/actions/playwright-run@v1
        with:

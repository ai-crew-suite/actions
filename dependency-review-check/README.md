# `@ai-crew-suite/actions/dependency-review-check`
...
yarn turbo run lint --filter=@ai-crew-suite/action-dependency-review-check
```
  ...
  dependency-review:
    ...
    steps:
      ...
      - name: 🔒 Run Centralized Dependency Review Check
        ...
        uses: ai-crew-suite/actions/dependency-review-check@v1
        with:
          fail-on-severity: "critical"

      - name: ⚠️ Log Security Compliance Exemption
        if: ${{ startsWith(github.head_ref, 'hotfix/') }}
        uses: ai-crew-suite/actions/dependency-review-bypass@v1
```
